...
> [!abstract]+ Canvas
> The [[SEAD Structure.canvas]] file groups tables into clusters of tables that connect to a single, central table. Connection lines between groups (rather than the individual tables) are also drawn.  With this approach some tables appear in multiple groups. Every table takes its colour from the folder in which it resides, making it easy to spot the connecting table(s) in two connected groups.
> 
...
*This illustration of the SEAD database structure was created using the note-taking program, [Obsidian.](https://obsidian.md/), chosen for its graphical representation of the connections between notes.*
